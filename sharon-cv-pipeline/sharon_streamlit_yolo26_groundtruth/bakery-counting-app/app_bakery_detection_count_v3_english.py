...
DEFAULT_CLASS_DISPLAY_MAPPING = {
    # Bakery product class currently used for the detection and counting task.
    # Additional classes can be added through bakery_class_display_mapping.json
...
st.markdown(
    ...
    <div class="sb-header">
        ...
        <h1>{APP_TITLE}</h1>
        <p>
            Upload bakery product images, run the YOLO model, review bounding boxes, count products in each image, and export an Excel report.
        </p>
    </div>
    """,
    unsafe_allow_html=True,
...
with st.sidebar:
    ...
    model_path_input = st.text_input(
        ...
        help=(
            "Path to the YOLO checkpoint trained for the bakery product classes."
        ),
    )
...
if upload_mode == "One Image":
    ...
    uploaded_files = st.file_uploader(
        "Choose One Bakery Image Folder",
        type=file_types,
...
render_message(
    (
        "Counting rule: each bounding box remaining after confidence filtering and NMS "
        "is counted as one bakery product. When multiple images contain the same tray, "
        "the system will count the products again because this is image-based counting, not tracking."
    ),
    "info",
)
...
with action_col_1:
    run_clicked = st.button(
        "Detect and Count Bakery Products",
        type="primary",
        width="stretch",
        disabled=(
            not upload_records
...
if run_clicked:
    ...
    try:
        ...
        with st.spinner("Loading the bakery detection model..."):
            bundle = load_model(
                model_path_input,
                mapping_path,
                mapping_mtime,
        ...
        render_message(
            (
                "Bakery product detection and counting are complete. "
                f"Successfully Processed Images: {len(successful_results)}/"
                f"{len(results)}."
            ),
            "success",
        )
        ...
        render_message(
            f"Processing Could Not Start: {error}",
            "error",
        )
...
def render_image_detail_fragment() -> None:
    ...
    if not successful_results:
        render_message(
            "There are no successfully processed images to display.",
            "error",
        )
        return
    ...
    if image_class_df.empty:
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "class_name": "Bakery Product Class",
                "confidence_threshold": (
                    "Confidence threshold"
                ),
                "quantity": "Quantity",
...
if results and report_payload:
    ...
    st.markdown(
        ...
        "Bakery Product Detection and Counting Results"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    with overview_tab:
        ...
        st.markdown(
            '<div class="sb-section-title">'
            "Total Quantity by Product Class"
            "</div>",
            unsafe_allow_html=True,
        )
        ...
        if overall_df.empty:
            render_message(
                "The model did not detect any bakery products in the uploaded images.",
                "warning",
            )
        else:
            ...
            with table_col:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "min_confidence": "Minimum confidence",
                        "max_confidence": "Maximum confidence",
                        "images_detected": "Images with Detections",
                    },
                )
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "total_detections": "Product Count",
    ...
    with download_tab:
        ...
        render_message(
            (
                "The Excel file includes product totals by class, per-image details, "
                "processing status, and inference settings."
            ),
            "info",
        )
    ...
    st.markdown(
        ...
        <div class="sb-card" style="margin-top: 24px;">
            ...
            <div class="sb-muted">
                1. Select the correct YOLO model trained for the bakery product classes.<br>
                2. Upload one image, multiple images, or an image folder.<br>
                3. Use confidence 0.25 and NMS IoU 0.55 as the initial settings.<br>
                4. Click <b>Detect and Count Bakery Products</b>, then review the bounding boxes.<br>
                5. Download the Excel, CSV, or ZIP package containing original and annotated images.<br><br>
                <b>Note:</b> Do not upload multiple images of the same tray when you need
                a unique total count, because image mode does not track objects across images.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
st.markdown(
    ...
    <div class="sb-footer">
        ...
        Static-image quantities are calculated from bounding boxes after confidence filtering and NMS.
    </div>
    """,
    unsafe_allow_html=True,
)
