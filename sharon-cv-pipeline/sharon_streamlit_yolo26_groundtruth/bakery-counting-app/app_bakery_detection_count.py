...
# =========================================================
APP_TITLE = "AI Bakery Product Detection & Counting"
...
CLASS_MAPPING_PATH = Path(
    os.getenv(
        "CLASS_MAPPING_PATH",
        str(APP_DIR / "bakery_class_display_mapping.json"),
    )
).expanduser()
...
CLASS_CONFIDENCE_PATH = Path(
    os.getenv(
        ...
        str(APP_DIR / "bakery_class_confidence_thresholds.json"),
    )
).expanduser()
...
LOGGER = logging.getLogger("sharon-bakery-counting")
logging.basicConfig(
    level=logging.INFO,
...
DEFAULT_CLASS_DISPLAY_MAPPING = {
    # Loại bánh đang dùng cho bài toán nhận diện và đếm bánh hiện tại.
    # Có thể bổ sung các class mới bằng file bakery_class_display_mapping.json
    # mà không cần sửa mã nguồn.
    "PA-CRO-0000054_MiniCroissant_Bakedd": "Mini Croissant (Baked)",
    "PA_CRO_0000054_MiniCroissant_Bakedd": "Mini Croissant (Baked)",
    "MiniCroissant_Bakedd": "Mini Croissant (Baked)",
    "MiniCroissant_Baked": "Mini Croissant (Baked)",
}
...
def resolve_display_names(
    ...
    for class_id, raw_name in raw_names.items():
        ...
        if display_name in used_names:
            display_name = f"{display_name}_Loại bánh{class_id}"
...
def render_groundtruth_evaluation_dashboard(
    ...
    if overall["missing_model_classes"]:
        render_message(
            (
                "Loại bánhes not represented in Ground Truth: "
                + " | ".join(
                    overall[
                        "missing_model_classes"
                    ]
...
def build_excel_file(
    ...
    with pd.ExcelWriter(
        ...
        overall_df.to_excel(
            writer,
            sheet_name="Overall by Loại bánh",
            index=False,
        )
        per_image_df.to_excel(
            writer,
            sheet_name="Per Ảnh by Loại bánh",
            index=False,
        )
        image_summary_df.to_excel(
            writer,
            sheet_name="Ảnh Summary",
            index=False,
        )
...
st.markdown(
    ...
    <div class="sb-header">
        ...
        <h1>{APP_TITLE}</h1>
        <p>
            Tải ảnh bánh, chạy mô hình YOLO, kiểm tra bounding box, đếm số bánh theo từng ảnh và xuất báo cáo Excel.
        </p>
    </div>
    """,
    unsafe_allow_html=True,
...
with st.sidebar:
    ...
    model_path_input = st.text_input(
        "Đường dẫn model bánh",
        value=DEFAULT_MODEL_PATH,
        help=(
            "Đường dẫn tới checkpoint YOLO đã train cho các class bánh."
        ),
    )
    ...
    confidence_mode = st.radio(
        "Chế độ confidence",
        options=[
            "Một threshold cho tất cả loại bánh",
            "Threshold riêng cho từng loại bánh",
        ],
        index=0,
        help=(
    ...
    confidence = st.slider(
        (
            ...
            if confidence_mode
            == "Một threshold cho tất cả loại bánh"
            else "Confidence mặc định / fallback"
        ),
        min_value=0.01,
        max_value=0.95,
    ...
    if (
        ...
        if not sidebar_model_path.is_file():
            ...
            try:
                ...
                with st.expander(
                    "Confidence theo từng loại bánh",
                    expanded=True,
                ):
    ...
    st.markdown(
        ...
        "IoU=0.55, end2end=False. In per-class mode, "
...
render_message(
    (
        "Quy tắc đếm: mỗi bounding box sau confidence filtering và NMS "
        "được tính là một bánh. Khi nhiều ảnh chứa cùng một khay bánh, "
        "hệ thống sẽ cộng lặp giữa các ảnh vì đây là image counting, không phải tracking."
    ),
    "info",
)
...
if enable_groundtruth_evaluation:
    if (
        ...
        if not groundtruth_source_ready:
            render_message(
                (
                    "Đánh giá với Ground Truth is enabled, "
...
if run_clicked:
    ...
    try:
        ...
        with st.spinner("Đang tải model nhận diện bánh..."):
            bundle = load_model(
                model_path_input,
                mapping_path,
                mapping_mtime,
        ...
        excel_path = (
            new_run_directories.reports
            / "bakery_count_report.xlsx"
        )
        zip_path = (
            new_run_directories.root
            / "Sharon_Bakery_Counting.zip"
        )
        ...
        render_message(
            (
                "Đã hoàn tất nhận diện và đếm bánh. "
                f"Ảnh xử lý thành công: {len(successful_results)}/"
                f"{len(results)}."
            ),
            "success",
        )
        ...
        render_message(
            f"Không thể bắt đầu xử lý: {error}",
            "error",
        )
...
def render_image_detail_fragment() -> None:
    ...
    if not successful_results:
        render_message(
            "Không có ảnh xử lý thành công để hiển thị.",
            "error",
        )
        return
...
if results and report_payload:
    ...
    with metric_2:
        st.metric(
            "Tổng số bánh phát hiện",
            metadata["total_detections"],
        )

    with metric_3:
        st.metric(
            "Số loại bánh phát hiện",
    ...
    with overview_tab:
        ...
        if overall_df.empty:
            render_message(
                "Model không phát hiện bánh trong các ảnh đã tải lên.",
                "warning",
            )
        else:
            ...
            with table_col:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "min_confidence": "Minimum confidence",
                        "max_confidence": "Maximum confidence",
                        "images_detected": "Số ảnh có phát hiện",
                    },
                )
    ...
    with download_tab:
        ...
        render_message(
            (
                "File Excel gồm tổng hợp số bánh theo loại, chi tiết từng ảnh, "
                "trạng thái xử lý và cấu hình inference."
            ),
            "info",
        )
        ...
        with csv_col_1:
            st.download_button(
                "CSV tổng hợp loại bánh",
                data=deferred_file_reader(
                    report_payload["overall_csv_path"]
                ),
                file_name="bakery_total_by_class.csv",
                mime="text/csv",
                on_click="ignore",
                width="stretch",
            )
        ...
        with csv_col_2:
            st.download_button(
                ...
                file_name="bakery_count_per_image.csv",
                mime="text/csv",
                on_click="ignore",
                width="stretch",
            )
        ...
        with csv_col_3:
            st.download_button(
                ...
                file_name="bakery_image_summary.csv",
                mime="text/csv",
                on_click="ignore",
                width="stretch",
            )
    ...
    st.markdown(
        ...
        <div class="sb-card" style="margin-top: 24px;">
            <div class="sb-section-title">Cách sử dụng</div>
            <div class="sb-muted">
                1. Chọn đúng model YOLO đã train cho bánh.<br>
                2. Tải một ảnh, nhiều ảnh hoặc một thư mục ảnh.<br>
                3. Giữ confidence 0.25 và NMS IoU 0.55 làm cấu hình khởi đầu.<br>
                4. Nhấn <b>Nhận diện và đếm bánh</b>, sau đó kiểm tra bounding box.<br>
                5. Tải Excel, CSV hoặc ZIP gồm ảnh gốc và ảnh đã gắn nhãn.<br><br>
                <b>Lưu ý:</b> Không tải nhiều ảnh chụp trùng cùng một khay nếu cần
                tổng số lượng duy nhất, vì chế độ ảnh không tracking xuyên ảnh.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
st.markdown(
    """
    <div class="sb-footer">
        Sharon Bakery · AI Bakery Detection & Counting ·
        Số lượng ảnh tĩnh được tính theo bounding box sau confidence filtering và NMS.
    </div>
    """,
    unsafe_allow_html=True,
)
